...
class ElevationGrid(
    ...
    fun renderHillshade(
        ...
        for (index in elevations.indices) {
            elevations[index] = bareEarth[index] + canopySpikes[index] * (1f - vegetationFilter.coerceIn(0f, 1f))
        }

        val analysisRadius = (featureScaleMeters.coerceAtLeast(cellDistance) / cellDistance).toInt().coerceIn(1, max(1, min(width, height) / 4))
        val local = if (visualizationMode == 3 || visualizationMode == 5) localStatistics(elevations, analysisRadius) else null
        val curvature = if (visualizationMode == 4 || visualizationMode == 5) curvature(elevations, cellDistance) else null
            ...
            for (x in 0 until width) {
                ...
                var color = when (visualizationMode.coerceIn(0, 8)) {
                    ...
                    5 -> {
                        ...
                        val score = ((residual * 0.58f + bend * 0.27f + roughness * 0.15f) * analysisSensitivity.coerceIn(0.4f, 2.5f)).coerceIn(0f, 1f)
                ...
                if (overlayType > 0 && overlayOpacity > 0f) {
                    historicalOverlayColor(x = x, y = y, elevations = elevations, currentElevation = elevations[index], overlayType = overlayType)?.let { overlay ->
    ...
    private fun hornGradient(elevations: FloatArray, x: Int, y: Int, cellDistance: Float, zScale: Float): Gradient {
        ...
        val z22 = at(x + 1, y + 1)
        return Gradient(
            dx = ((z02 + 2f * z12 + z22) - (z00 + 2f * z10 + z20)) / (8f * cellDistance) * zScale,
            dy = ((z20 + 2f * z21 + z22) - (z00 + 2f * z01 + z02)) / (8f * cellDistance) * zScale,
        )
    }
    ...
    private fun multiDirectionalShade(dx: Float, dy: Float, azimuth: Float, altitude: Float): Float {
        ...
        return (primary * 0.45f + right * 0.2f + opposite * 0.15f + left * 0.2f).coerceIn(0f, 1f)
    }
    ...
    private fun curvature(elevations: FloatArray, cellDistance: Float): FloatArray {
        ...
        for (y in 0 until height) {
            for (x in 0 until width) {
                ...
                output[y * width + x] = (elevations[index(x - 1, y)] + elevations[index(x + 1, y)] + elevations[index(x, y - 1)] + elevations[index(x, y + 1)] - 4f * center) / divisor
            }
        }
        return output
    }
    ...
    private fun isContourPixel(elevations: FloatArray, x: Int, y: Int, interval: Float): Boolean {
        ...
        return (x > 0 && floor(elevations[index(x - 1, y)] / interval).toInt() != level) || (y > 0 && floor(elevations[index(x, y - 1)] / interval).toInt() != level)
    }

    private fun historicalOverlayColor(x: Int, y: Int, elevations: FloatArray, currentElevation: Float, overlayType: Int): Int? {
        if (overlayType == 1) {
            ...
            val isContour = currentElevation.toInt() % 12 == 0 && (elevations[index(x - 1, y)].toInt() % 12 != 0 || elevations[index(x, y - 1)].toInt() % 12 != 0)
    ...
    private fun divergingReliefColor(value: Float): Int {
        ...
        return if (value < 0f) blend(neutral, Color.rgb(32, 116, 210), -value) else blend(neutral, Color.rgb(232, 82, 30), value)
    }

    private fun divergingCurvatureColor(value: Float): Int {
        val neutral = Color.rgb(190, 190, 184)
        return if (value < 0f) blend(neutral, Color.rgb(24, 180, 196), -value) else blend(neutral, Color.rgb(208, 48, 146), value)
    }
    ...
    private fun index(x: Int, y: Int): Int = y.coerceIn(0, height - 1) * width + x.coerceIn(0, width - 1)

    private fun rectangleSum(integral: DoubleArray, stride: Int, x0: Int, y0: Int, x1: Int, y1: Int): Double = integral[(y1 + 1) * stride + x1 + 1] - integral[y0 * stride + x1 + 1] - integral[(y1 + 1) * stride + x0] + integral[y0 * stride + x0]
